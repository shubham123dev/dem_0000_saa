...
export class WorkplaceAgentApiService {
  ...
  query(id:string,query:string):Observable<AgentQueryResponse>{ const body=agentQueryRequestSchema.parse({query}); return this.client.request('POST',`${orgPath(id)}/agent/query`,agentQueryResponseSchema,{body}); }
  ...
  listProposals(id:string,filters:AgentActionListFilters={}):Observable<AgentActionProposalListResponse>{ const parsed=agentActionListFiltersSchema.parse(filters); let params=new HttpParams(); if(parsed.status)params=params.set('status',parsed.status); if(parsed.actionName)params=params.set('action_name',parsed.actionName); if(parsed.requestedBy)params=params.set('requested_by',parsed.requestedBy); if(parsed.limit)params=params.set('limit',String(parsed.limit)); if(parsed.cursor)params=params.set('cursor',parsed.cursor); return this.client.request('GET',`${orgPath(id)}/agent/actions`,actionProposalListResponseSchema,{params}); }
