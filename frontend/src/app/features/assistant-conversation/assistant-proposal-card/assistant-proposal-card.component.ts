...
export class AssistantProposalCardComponent implements OnChanges {
  ...
  readonly control=inject(ProposalControlFacade);
  ...
  confirm():void{const value=this.decision();if(!value||!this.canConfirm()||this.control.busy())return;const reason=this.reason().trim()||null;const confirmation=this.confirmation().trim()||null;const request=value==='approve'?this.control.approve(reason,confirmation):value==='reject'?this.control.reject(reason):value==='execute'?this.control.execute(confirmation):this.control.cancel(reason);request.subscribe({next:()=>this.decision.set(null),error:()=>undefined});}
}
