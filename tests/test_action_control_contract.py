...
from app.db.agent_run_models import AgentConversationORM, AgentRunORM


async def test_conversation_action_control_resolves_exact_proposal_with_utc_timestamps(
    client: AsyncClient,
    ...
    db_session.add(
        AgentRunORM(
            ...
            user_message_id="message-contract-test",
    ...
    db_session.add(
        AgentActionProposalORM(
            ...
            arguments_json={"email": "contract@example.test", "display_name": "Contract User", "role": "sandbox_reader"},
            changes_json=[{"field": "organization_membership", "before": None, "after": "invited sandbox_reader"}],
            ...
            approval_policy_json={"self_approval_allowed": True, "required_approver_permission": "agent.actions.approve", "minimum_approvals": 1},
    ...
    response = await client.get(
        f"/workplace/organizations/org_sandbox_001/agent/control/conversations/{conversation_id}/action",
